...
{
	auto query = conn->ComposeQuery("INSERT INTO `user` (`region`, `account_id`, `summoner_id`, `summoner_name`, `register_date`, `update_date`) VALUES ('%s', %ld, %ld, '%s', NOW(), NOW()) ON DUPLICATE KEY UPDATE `update_date` = VALUES(`update_date`)", region, accountId, summonerId, summonerName);
